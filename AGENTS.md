...
- Native rebuild is automated: `npm test` runs a `pretest` hook that rebuilds for system Node; `npm run dev` rebuilds for Electron's ABI.
...
- Unit tests: `npm test` (auto-rebuilds `better-sqlite3` for system Node via the `pretest` hook).
